...
const Login = () => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // SC-SECURITY-01: Reemplaza historial para evitar volver a /login con botón atrás
  ...
  return (
    <div className="min-h-screen bg-cjdg-darker flex items-center justify-center p-4">
      {/* Background Glow */}
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-96 h-96 bg-cjdg-primary/20 blur-[100px] rounded-full pointer-events-none"></div>

      <div className="w-full max-w-md relative z-10 glass-panel p-8">

        <div className="text-center mb-8">
          <h1 className="text-3xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-white to-cjdg-accent">Proyectos CJDG</h1>
          <p className="text-cjdg-textMuted mt-2 text-sm uppercase tracking-widest font-mono">Portal de Ingreso</p>
        </div>

        {error && (
          <div className="bg-red-500/10 border border-red-500/50 text-red-200 p-3 rounded-md mb-6 flex items-start gap-3 text-sm">
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                required
                minLength={4}
                className="block w-full pl-10 pr-3 py-2 border border-cjdg-border rounded-md bg-cjdg-dark/50 text-white placeholder-cjdg-textMuted focus:outline-none focus:ring-2 focus:ring-cjdg-primary focus:border-transparent transition-all"
        ...
        {/* OAuth Buttons — UI lista, sin funcionalidad real */}
        <div className="mt-6">
          <div className="relative">
            ...
            <div className="relative flex justify-center text-xs">
              <span className="px-3 bg-cjdg-dark text-cjdg-textMuted">o continúa con</span>
            </div>
          </div>
        ...
        <div className="mt-8 text-center text-sm text-cjdg-textMuted border-t border-white/5 pt-6">
          ¿Representas una empresa nueva?{' '}
          <Link to="/register" className="text-cjdg-accent hover:text-white transition-colors font-medium">
